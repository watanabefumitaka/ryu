...
STATE_INIT_FLOW_TESTER = 13
STATE_FLOW_INSTALL_TESTER = 14
STATE_FLOW_EXIST_CHK_TESTER = 15
STATE_GET_FLOW_STATS = 16
...
class OfTester(app_manager.RyuApp):
    ...
    def _test_execute(self, test, description):
        ...
        try:
            ...
            for pkt in test.tests:
                ...
                # 2.1. Get stats before sending packet(s).
                ...
                if KEY_EGRESS in pkt or KEY_PKT_IN in pkt:
                    ...
                    end = self._test(STATE_GET_FLOW_STATS)
    ...
    def _test(self, state, *args):
        test = {STATE_INIT_FLOW: self._test_initialize_flow,
                ...
                STATE_GET_FLOW_STATS: self._test_get_throughput,
    ...
    def _test_initialize_flow_tester(self):
        xid = self.tester_sw.del_test_flow()
        self.send_msg_xids.append(xid)
    ...
    def _test_flow_matching_check(self, pkt):
        ...
        # 2. check the SW which sended PacketIn and output packet.
